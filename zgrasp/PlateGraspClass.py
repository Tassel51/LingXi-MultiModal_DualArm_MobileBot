...
from zgrasp.camera import RealSenseCamera  # 修改这里导入适合计算机自带摄像头的相机类
from zgrasp.camera_data import CameraData
from zgrasp.function import detect

from zgrasp.device import get_device
...
class Grasp:
    def __init__(self):
        ...
        # 连接到相机
        self.cam = RealSenseCamera(device_id="932122061130")  # D435
        self.cam.connect()
    ...
    def parse_args(self):
        parser = argparse.ArgumentParser(description='zgrasp')
    ...
    def get_location_p(self):
        locations = []
        location = np.zeros((1, 3))  # 初始化列表存储所有location
        start_time = time.time()  # 记录开始时间
        while time.time() - start_time < 5:  # 循环运行2秒
            image_bundle, depth_intrin, aligned_depth_frame = self.cam.get_image_bundle()
            rgb = image_bundle['rgb']  # 这里返回的是BGR图
            rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)
            depth = image_bundle['aligned_depth']
