...
class SerialArmControllerClass:
    ...
    def coord_ctrl(self, target_x: float = 277, target_y: float = -14, target_z: float = 277,
                   theta_angle: float = 90, grabber_angle: float = 170) -> None:
        """
        逆运动学解算机械臂位置（xyz坐标系）
        COORD_CTRL: T:cmdType, P1 - 3: coordInput, P4: thetaAngle, P5: grabberAngle, S1: stepDelay, S5: grabberSpeed
        {"T": 2, "P1": 277.5104065, "P2": -13.75, "P3": 276.5822754, "P4": 90, "P5": 180, "S1": 10, "S5": 200}
        ...
        if self.__send_command_frame(command_frame):
            ...
            try:
                data = self.__serial_entity.read_until(b'\n')
                print("Raw data:", data)
                data_str = data.decode('utf-8').strip()  # Ensure we strip out any extra whitespace or newline characters
                if data_str.startswith('{') and data_str.endswith('}'):  # Check if it's a valid JSON string
                    json_data = json.loads(data_str)
                else:
                    print(f"接收到的非JSON数据：{data_str}")
               # json_data = json.loads(data.decode('utf-8'))
                ...
                print(f"接收到的非JSON数据：{data_str}")
                json_data = None  # 确保 json_data 被定义，避免 UnboundLocalError
        ...
        self.__serial_entity.flush()  # 确保清除串口缓冲区
        ...
        if self.__send_command_frame(command_frame):
            ...
            json_data = None
            try:
                data_str = data.decode('utf-8').strip()  # 去掉多余的换行符或空格
                if data_str.startswith('{') and data_str.endswith('}'):  # 检查是否为有效的 JSON 字符串
                 json_data = json.loads(data_str)
                else:
                    print(f"接收到的非JSON数据：{data_str}")

                if json_data:  # 确保 json_data 不是 None
                    print("JSON 数据：", json_data)
                    self.__coord_x = json_data.get("P1", target_x)
                    self.__coord_y = json_data.get("P2", target_y)
                    self.__coord_z = json_data.get("P3", target_z)
                    self.__coord_t = json_data.get("P4", theta_angle)
                    self.__coord_g = json_data.get("P5", grabber_angle)
                else:
                    print("JSON 数据无效，使用默认值。")
                # 如果 JSON 数据无效，使用默认值
                    self.__coord_x = target_x
                    self.__coord_y = target_y
                    self.__coord_z = target_z
