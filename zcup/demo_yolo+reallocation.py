...
if __name__ == '__main__':
    ...
    while True:
        ...
        annotated_frame = results[0].plot() # 将 YOLOv8 检测结果（边界框、类别标签、置信度）绘制到原始图像上，生成标注后的图像

        for r in results:
            annotated_frame,y = cup_detect(results, r, annotated_frame, rgb, aligned_depth_frame, depth_intrin)
...
if __name__ == '__main__':
    ...
    # 连接摄像头
    cam = RealSenseCamera(device_id="932122061130")
    cam.connect()
    ...
    while True:
        # 获取图像数据
        image_bundle, depth_intrin, aligned_depth_frame = cam.get_image_bundle()
        rgb = cv2.cvtColor(image_bundle['rgb'], cv2.COLOR_BGR2RGB)
        depth = image_bundle['aligned_depth']
        x, depth_img, rgb_img = cam_data.get_data(rgb=rgb, depth=depth)

        # 运行 YOLO 检测
        results = model(rgb, vid_stride=15, retina_masks=True)
        annotated_frame = results.plot()  # 初始标注图像

        # 处理杯子检测结果
        annotated_frame, locations = cup_detect(
            results, annotated_frame, rgb, aligned_depth_frame, depth_intrin
        )
